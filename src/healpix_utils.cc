...
void mapLonLatToHEALPixCell(
  ...
  */
  ...
  auto healpix_lonlat =
      atlas::array::make_view<double, 2>(healpix_cell_fs.lonlat());
  auto healpix_gidx =
      atlas::array::make_view<atlas::gidx_t, 1>(healpix_cell_fs.global_index());
  ...
  auto ifs_fs = fieldToMap.functionspace();
  auto ifs_lonlat = atlas::array::make_view<double, 2>(ifs_fs.lonlat());
  // Halo: value 0 everywhere except in halo cells
  auto ifs_ghost = atlas::array::make_view<int, 1>(ifs_fs.ghost());
  // Resize the mapping vector to the size of ifs grid points
  mappingVector.resize(fieldToMap.size());
  // Search for the closest cell in the HEALPix mesh
  for (atlas::idx_t j = 0; j < ifs_lonlat.shape(0); ++j) {
    ...
    atlas::PointLonLat p{ifs_lonlat(j, 0), ifs_lonlat(j, 1)};
    auto closest = search.closestPoint(p);
    mappingVector[j] = closest.payload();
  }

  // Now map the vertices lonlat coordinates of each cell
  auto healpix_nodes_lonlat =
      atlas::array::make_view<double, 2>(HPmesh.nodes().lonlat());
  ...
  for (atlas::idx_t jcell = 0; jcell < HPmesh.cells().size(); ++jcell) {
    ...
    for (atlas::idx_t jnode = 0; jnode < nb_nodes_per_cell; ++jnode) {
      ...
      cellVertices[jcell].push_back(
          atlas::PointLonLat{healpix_nodes_lonlat(nodes[jnode], 0),
                             healpix_nodes_lonlat(nodes[jnode], 1)});
    }
  }
}
...
cellToPolygons(std::vector<int> &ee_indices, std::vector<int> &mapping,
  ...
  for (const int &point_idx : ee_indices) {
  ...
  for (const int &cell_idx : ee_cells) {
    for (size_t vidx = 0; vidx < vertices[cell_idx].size(); ++vidx) {
      ...
      std::pair<atlas::PointLonLat, atlas::PointLonLat> ee_edge = {
          vertices[cell_idx][vidx],
          vertices[cell_idx][(vidx + 1) % vertices[cell_idx].size()]};
      if (count_edges.find(ee_edge) != count_edges.end()) {
        ++count_edges[ee_edge];
      } else if (count_edges.find({ee_edge.second, ee_edge.first}) !=
                 count_edges.end()) {
  ...
  for (auto &ee_edge : count_edges) {
    ...
    auto it = std::lower_bound(vertex_walk[ee_edge.first.first].begin(),
                               vertex_walk[ee_edge.first.first].end(),
  ...
  while (!vertex_walk.empty()) {
    ...
    if (vertex_walk.find(currentPoint) != vertex_walk.end()) {
      nextPointIdx = 0;
      if (vertex_walk[currentPoint].size() > 1 &&
          currentPoint[0] > vertex_walk[currentPoint][0][0]) {
      ...
      if (vertex_walk[previousPoint].size() == 1) {
        ...
        vertex_walk[previousPoint].erase(vertex_walk[previousPoint].begin() +
                                         previousPointIdx);
      }
